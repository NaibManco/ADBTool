...
│  ├─ embedded-mirror-manager.ts  投屏：@yume-chan(scrcpy 4.0) 视频流会话与控制注入（内嵌面板与独立窗口共用）
...
├─ mirror.md           投屏：内嵌视频流 + 独立窗口（共用 @yume-chan 会话）
...
- 两种投屏形态，共用每设备一个的 @yume-chan 会话（3.0.0-beta.2 + 内置 scrcpy-server 4.0，WebCodecs 硬解，画布支持点击/拖动/滚轮、右键=返回）：「启动投屏」内嵌到主窗口右侧工作区；投屏面板头部的「弹出」切换到独立大屏窗口（同会话流渲染到独立 BrowserWindow），弹出时自动关闭内嵌投屏。不再依赖外部 scrcpy.exe（其 SDL 显示层在部分机器必然段错误，已移除）。
...
打包版本优先使用 `resources\scrcpy` 中内置的 adb.exe（该目录还带 scrcpy-server 供投屏/录屏/剪贴板同步使用）。开发环境还支持：
...
- 打包后确认 EXE 存在且非空，并检查 SHA-256、签名状态以及以下内置资源：`adb.exe`、`scrcpy-server`、`AdbWinApi.dll`、`AdbWinUsbApi.dll`。
...
- `vendor\scrcpy` 通过 electron-builder 的 `extraResources` 打包（filter 只带 adb.exe、AdbWinApi.dll、AdbWinUsbApi.dll、scrcpy-server），因此正常分享单个 portable EXE 时不要求接收方另装 ADB。
